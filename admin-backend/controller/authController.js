...
export const login=async (req,res,next)=>{
   try{
      ...
      if(admin){
        ...
        res.cookie("admin_user",token,{expires:new Date(Date.now()+2592000000),httpOnly:false,secure:process.env.NODE_ENV === 'production',sameSite:process.env.NODE_ENV === 'production' ? 'none' : 'lax'}).status(200).json({details:{_id,email,admin_type}})
